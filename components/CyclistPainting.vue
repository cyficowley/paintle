<template>
    <div ref="painting-container"
        class="flex justify-center items-center h-full overflow-hidden">
        <div class="relative bg-white"
            :style="{ width: imageScaledDimensions.width + 'px' }"
            style="font-size: 0">
            <CyclistPaintingChunk v-for="(imagePiece, index) in imagePieces"
                :key="imagePiece.id" class="inline-block"
                :painting-piece="imagePiece"
                :color="colors[index % colors.length]" :changeable="changeable"
                :reveal="reveal" @revealed="handleReveal" />
        </div>
    </div>
</template>
    ...
    computed: {
        filteredPaintings() {
            const filteredPaintings = this.allAnswers.filter((painting) => {
                ...
                return (
                    painting.name.toLowerCase().includes(this.message.toLowerCase())
                )
            })
