<template>
    <div class="flex flex-col h-screen overscroll-none">
        <CyclistHeader class="mb-4" :won="won" :lost="lost"
            @directions="showDirections = true" @winner="winPopup = true"
            @loser="losePopup = true" />
        <div class="container flex-grow min-h-0 mx-auto">
            <CyclistPainting :painting="selectedAnswer" :changeable="!won"
                :reveal="won || lost" @revealed="handleReveal" />
        </div>
        <div
            class="container grid grid-cols-12 gap-4 px-2 py-2 mx-auto sm:py-4">
            <CyclistGuesser :all-answers="possibleAnswers"
                :can-guess="canGuess && !won" :revealed="revealed"
                class="col-span-12 sm:col-span-8 sm:col-start-3"
                :wrong-guess="wrongGuess" :won="won" :lost="lost"
                @guessed="handleGuess" @loser="handleLoser" />
        </div>
...
import Cyclist_Directions from '~/components/CyclistDirections.vue'
